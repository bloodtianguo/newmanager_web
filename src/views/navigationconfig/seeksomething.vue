<template>
  <div class="app-container">
    <!-- 新增找人找物 -->
    <div class="TitTopfon Tittop">导航配置-找人找物</div>
    ...
    <el-form :model="addform"
      ...
      <el-card shadow="hover"
        ...
        <div>
          ...
          <el-row :gutter="24"
                  v-for="(exitem,index) in circu_first_ex"
                  :key="index">
            <!-- 循环 -->
      ...
      <el-card shadow="hover"
        ...
        <el-row :gutter="24"
          ...
          <el-col :span="11"
                  :offset="0"
                  v-for="(aditemsecond,index) in addform.arraySecond"
                  :key="index">
            <el-divider content-position="center"
                        class="divider_box">
              ...
              <el-form-item label="主题图"
                ...
                <div class="upload_box">
                  ...
                  <el-upload accept="image/jpeg,image/png"
                             ...
                             :show-file-list="false"
                             :headers="myHeaders"
                             :on-success="dynamicPicSuccess2.bind(null, {'index':index,'data':aditemsecond})"
...
export default {
  ...
  // },
  created () {
    this.Seekinfostate = this.$store.state.user.Seekinfostate
    this.getDetail()
    this.initshow()
  },
  data () {
  ...
  // 监听vuex中Seekinfostate变化
  computed: {
    getStoreItem () {
      return this.$store.state.user.Seekinfostate
    }
  },
  watch: {
    getStoreItem () {
  ...
  methods: {
    ...
    addhome () {
      addHomeBars(this.homeBar).then(response => {
        if (response.code == "0") {
          Success("添加成功!")
          this.$parent.SeekshowClo();
          this.$store.dispatch('user/setSeekPopupval', false)
        } else {
    ...
    // 从列表详情获取详情
    getDetail () {
      if (this.Seekinfostate == "详情" || this.Seekinfostate == "编辑") {
        if (sessionStorage.homeId) {
