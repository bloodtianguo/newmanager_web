...
<div>
    ...
    <div class="recreation-container" v-if="currentPageIsShow">
        ...
        <div class="head">
            ...
            <div>
                <span>视频标题：</span>
                <el-input clearable v-model="searchVideoTitle" placeholder="输入视频标题进行搜索"
                 class="search-value" ></el-input>
            </div>
            <div>
            ...
            <el-table
                ...
                >
                    <template slot-scope="scope">
                        ...
                        <el-button  class="el-btn-mini" type="primary" size="mini" @click="openSelectMark(scope.row)">标签</el-button>
...
import { cardVideoListAPI,updateShelvesAPI,batchUpdateShelvesAPI, addOfficealVideoAPI } from '@/api/content/recreation'
export default {
    components:{
        SelectMark,
        uploadcover
    ...
    data(){
      ...
      videocoverid:0,
            ...
            filetext:'打卡视频',
            filename:'打卡视频',
            tHeader:['短视频ID','类别','用户ID','用户昵称','短视频标题','被打赏数','被点赞数','发布时间','内容得分','上下架状态','下架原因'],
            filterVal:['videoId','name','userId','nickName','title','reward','likeNum','createTimeFormat','score','shelvesStatusKey','mark'],
        }
    },
    mounted(){
    ...
    coverClose(val){
      this.coverVisible=val
        ...
        async getData(){
            ...
            let res = await cardVideoListAPI(params)
        ...
        goVideoDetails({videoId}){
            ...
            this.$router.push({path:'/videoContentManage/card/videoDetails',query:{videoId}})
        },
    ...
            }
        ...
        currentPageHandle(){
            this.currentPageIsShow = this.$route.path == '/videoContentManage/card'
        }
